...
class PostsManager(models.Manager):
    def check(self,content,id)

        messages = []

        if len(content) < 1:
            messages.append('Secret to short, please try again')
        else:
            secret = Posts.objects.create(secret=secret, createdby_id=User.objects.get(id=id))
    
    def liked(self, secret_id, user_id):
        post = self.get(id=secret_id)
        likeBy = User.objects.get(id=user_id)
        userLiked = User.objects.filter(likes=secret,id=user_id)

        if len(user_likes) != 0:
            messages.append('You cannot like a post you already liked!')
            return False, Messages
        else:
            secret.like.add(likedBy)
            return True